...
/// LLM provider + Postgres memory backend + memory tools.
/// Postgres runs in a Testcontainer; LLM tests auto-skip when no local server is available.
/// </summary>
[TestFixture]
[Category("Integration")]
public sealed class PostgresMemoryE2ETests
{
    ...
    // Current test's memory instance
    private PostgresMemory _memory = null!;
    ...
    public async Task ClearPreservesHistory_ToolVerification()
    {
        ...
        // But history still exists in Postgres (WORM semantics)
        await using var conn = new NpgsqlConnection(_connectionString);
        await conn.OpenAsync();
    ...
    // ── LLM + Postgres Memory Tests (auto-skip when LLM unavailable) ──
