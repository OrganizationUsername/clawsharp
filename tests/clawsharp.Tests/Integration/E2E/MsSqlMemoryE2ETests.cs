...
/// LLM provider + MsSql memory backend + memory tools.
/// SQL Server runs in a Testcontainer; LLM tests auto-skip when no local server is available.
/// </summary>
[TestFixture]
[Category("Integration")]
public sealed class MsSqlMemoryE2ETests
{
    ...
    private MsSqlMemory _memory = null!;
    ...
    public async Task ClearPreservesHistory_ToolVerification()
    {
        ...
        // But history still exists in SQL Server (WORM semantics)
        await using var conn = new SqlConnection(_connectionString);
        await conn.OpenAsync();
    ...
    // ── LLM + MsSql Memory Tests (auto-skip when LLM unavailable) ──
