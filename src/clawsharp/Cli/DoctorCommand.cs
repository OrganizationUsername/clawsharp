...
public sealed class DoctorCommand : AsyncCommand<DoctorCommand.Settings>
{
    ...
    public override async Task<int> ExecuteAsync(CommandContext context, Settings settings, CancellationToken cancellationToken)
    {
        ...
        CheckProvider(config, ref failures, ref warnings);
        CheckMemory(config, ref failures, ref warnings);

        var workspace = ConfigLoader.ExpandHome(config.Tools.Workspace);
        CheckWorkspace(workspace, ref warnings);
        CheckChannels(config, ref warnings);
        CheckHeartbeat(config, ref warnings);
    ...
    private static void CheckProvider(AppConfig config, ref int failures, ref int warnings)
    {
    ...
    private static void CheckMemory(AppConfig config, ref int failures, ref int warnings)
    {
    ...
    private static void CheckWorkspace(string workspace, ref int warnings)
    {
    ...
    private static void CheckChannels(AppConfig config, ref int warnings)
    {
    ...
    private static void CheckHeartbeat(AppConfig config, ref int warnings)
    {
        if (config.Agents.Defaults.Heartbeat is not { Enabled: true } heartbeat)
        {
            return;
        }
    ...
    private static async Task<int> CheckProviderConnectivity(
        AppConfig config, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var pingReq = new ChatRequest(
                ...
                Temperature: 0,
                MaxTokens: 1);
            await p.ChatAsync(pingReq, cancellationToken).ConfigureAwait(false);
    ...
    private static async Task<int> CheckDatabaseConnectivity(
        AppConfig config, CancellationToken cancellationToken)
    {
    ...
    private static async Task<int> CheckWorkspaceWritability(
        string workspace, CancellationToken cancellationToken)
    {
        try
        {
            var testFile = Path.Combine(workspace, ".clawsharp-write-test");
            await File.WriteAllTextAsync(testFile, "", cancellationToken).ConfigureAwait(false);
    ...
    private static void CheckSystemMd(string workspace, ref int warnings)
    {
    ...
    private static void CheckBraveSearch(AppConfig config, ref int warnings)
    {
